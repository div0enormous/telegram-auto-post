...
def create_button_layout(buttons):
    """
    Create a grid layout for buttons (2-3 buttons per row)
    Similar to the second image layout
    """
    if not buttons:
        return None
    ...
    for i, btn in enumerate(buttons):
        ...
        # Add row when it has 2 buttons, or if it's the last button
...
async def delete_post_command(client, message: Message):
    ...
    if not posts:
        ...
        return
    buttons = [[InlineKeyboardButton(f"🗑️ {title}", callback_data=f"delete_post_{post_id}")] for post_id, title, _ in posts]
...
async def handle_messages(client, message: Message):
    ...
    if not is_admin(user_id):
        if message.text:
            ...
            if not results:
                await client.send_message(message.chat.id, "😕 No results found for your query.")
            else:
                buttons = [[InlineKeyboardButton(title.replace('*', ''), callback_data=f"view_post_{post_id}")] for post_id, title in results]
                await client.send_message(message.chat.id, "🔎 **Here are the search results:**", reply_markup=InlineKeyboardMarkup(buttons))
        else:
